...
#define bit_Set(Reg, Num) Reg |= (1<<Num)  // This is used to set a special bit in the register.
#define bit_Clr(Reg, Num) Reg &= ~(1<<Num) // This is used to clear a special bit a register.
...
{
	// write this function
		while(GPIO_PORTF_DATA_R & (1<<4)); // wait here as long as AS is High
	// reaching here means AS is low so .. End the waiting function and go back.
}
...
void WaitForASHigh(void)
{
	// write this function
	while(!(GPIO_PORTF_DATA_R & (1<<4))); // wait here as long as AS is High
	// reaching here means AS is fallen so .. End the waiting function and go back.
}
